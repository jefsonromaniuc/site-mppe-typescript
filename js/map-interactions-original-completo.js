...
class MapInteractionsOriginalCompleto {
    ...
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                yield window.dataLoader.loadData();
    ...
    createMunicipioPoint(municipio, dados) {
    ...
    handlePointHover(e, municipio, dados) {
        // Efeito visual no ponto
        const point = e.target;
        point.style.transform = 'translate(-50%, -50%) scale(1.5)';
        point.style.zIndex = '20';
        // Mostrar tooltip
        this.showTooltip(e, municipio, dados);
    }
    showTooltip(e, municipio, dados) {
        if (!this.tooltip)
            return;
        const hasPromotorias = dados.promotorias && dados.promotorias.length > 0;
    ...
    handlePointClick(e, municipio, dados) {
        e.stopPropagation();
        this.selectMunicipio(municipio, dados);
    }
    selectMunicipio(municipio, dados) {
    ...
    filterMunicipiosByEntrancia(entrancia) {
        ...
        if (entrancia && entrancia.startsWith('vaga-')) {
            if (mapContainer)
                mapContainer.classList.add('filtering-vagas');
        }
        else {
            if (mapContainer)
    ...
    isPromotoriaVaga(promotoria) {
        ...
        const isEmptyOrVaga = !titular ||
            titular.trim() === '' ||
            vagaKeywords.some(keyword => titular.toUpperCase().includes(keyword));
    ...
    setupEventListeners() {
        ...
        if (mapWrapper) {
            mapWrapper.addEventListener('click', (e) => {
                if (e.target === mapWrapper || e.target.id === 'mapa-pernambuco') {
                    this.clearSelection();
                }
            });
        }
    ...
    showPromotoriaModal(promotoria) {
